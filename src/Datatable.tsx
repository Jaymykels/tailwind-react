...
const Datatable: React.SFC<DatatableInterface> = ({ currentData, currentPage, lastPage, pages, dispatch, header }) => {
  ...
  return (
    <div className="m-auto pt-2 antialiased font-sans font-serif font-monotext-center">
          ...
          <table className="w-full table-auto border-collapse whitespace-no-wrap">
            <thead>
              <tr>
                {
                  header.map(col =>
                    <th key={col.key} className="py-4 px-6 bg-gray-200 font-bold uppercase text-sm text-gray-600 border-b border-gray-500">
        ...
        {/* Pagination */}
        {
          ...
          <div className="flex flex-wrap justify-between pt-3">
            ...
            <ul className="inline-flex list-inside rounded">
              <li>
                <button
                  className="focus:outline-none block text-white hover:bg-indigo-700 bg-indigo-500 px-3 py-2 mr-1 border rounded"
                ...
                pages.map((page, index) =>
                  <li key={page}>
                    <button
                      ...
                      className={`focus:outline-none block text-white hover:bg-indigo-700 ${currentPage === page ? "bg-indigo-700" : "bg-indigo-500"} px-3 py-2 ${index === 0 ? "rounded-l border-l" : index === (pages.length - 1) && "rounded-r border-r"}`}>
                      {page}
                    </button>
                  </li>
                )
              ...
              <li>
                <button
                  className="focus:outline-none block text-white hover:bg-indigo-700 bg-indigo-500 px-3 py-2 ml-1 border rounded"
